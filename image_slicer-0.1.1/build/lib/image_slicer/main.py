...
def calc_columns_rows(n, imagename="none"):
    """
    ...
    if imagename != "none":
        ...
        im_w, im_h = im.size
        num_columns = int(ceil(im_w/128.0))
        num_rows = int(ceil(im_h/128.0))
    ...
    #else:
    #num_rows = int(ceil(sqrt(n)))
    #num_columns = int(ceil(n / float(num_rows)))
...
def slice(filename, number_tiles, save=True):
    ...
    im_w, im_h = im.size
    columns, rows = calc_columns_rows(number_tiles,filename)
